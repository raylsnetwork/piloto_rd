...
async function example13() {
    ...
    const destinationAcc = process.env.DEST_CLIENT_ACC ?? "";
    ...
    const opData = {
            operationId: process.env.TPFT_OP_ID ?? "",
            chainIdSeller: chainIdDestination, // ID vendedor, tpft owner
            chainIdBuyer: chainId, // ID comprador, cbdc owner
            accountSeller: destinationAcc, // Endereço do vendedor
            accountBuyer: clientSigner.address, // Endereço do comprador
            ...
            isBetweenClients: true // Se é, ou não, entre clientes
        };
    ...
    const txOpReg = await tpftOpContract.callRegisterOperation(opData, wdResourceId);
    await txOpReg.wait();
